...
class OurTrainer(Trainer):
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        # For parallel ZO, all ranks must see the same data (perturbation parallelism, not data parallelism)
        # Use UniformWithReplacementSampler with deterministic seed on all ranks
    ...
    def _inner_training_loop(
        ...
        if self.args.dpzero:
            if self.args.dp_epsilon > 0:
                multiplier = get_noise_multiplier(target_epsilon=self.args.dp_epsilon* 0.95,
        ...
        # Setup parallel ZO (distZO2 integration)
        if dist.is_initialized() and dist.get_world_size() > 1:
            from .distzo import ParallelZOOptimizer
            zo_opt = ParallelZOOptimizer(
                model, lr=self.args.learning_rate, eps=self.args.zo_eps,
                weight_decay=self.args.weight_decay, device=args.device
            )
            is_parallel_zo = True

            # Broadcast noisy_batch_size from rank 0 to ensure consistency across ranks
            if self.args.dpzero and self.args.dp_epsilon > 0:
                nbs_tensor = torch.tensor([noisy_batch_size], device=args.device, dtype=torch.float32)
                dist.broadcast(nbs_tensor, src=0)
                noisy_batch_size = nbs_tensor.item()
                self.dpzero_gaussian_std = multiplier * self.args.dpzero_clip_threshold / noisy_batch_size

            if zo_opt.rank == 0:
                print(f'[Parallel ZO] World size: {zo_opt.world_size}, Num pairs: {zo_opt.num_pairs}')
        else:
            zo_opt = None
            is_parallel_zo = False
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if is_parallel_zo:
                    # === Parallel gradient computation via distZO2 ParallelZOOptimizer ===
                    # Deterministic random seeds across all ranks (same base seed + global step)
                    step_rng = np.random.RandomState(self.args.seed + self.state.global_step)
                    all_random_seeds = step_rng.randint(1000000000, size=self.args.n)

                    # Distribute K directions across GPU pairs
                    my_directions = zo_opt.distribute_directions(self.args.n)
                    local_grads = []

                    for d_idx in my_directions:
                        random_seed = int(all_random_seeds[d_idx])

                        # Perturb: role 0 -> +eps*z, role 1 -> -eps*z
                        zo_opt.perturb_parameters_by_named(self.named_parameters_to_optim, random_seed)

                        # Each GPU does ONE forward pass (within-pair parallelism)
                        loss_local = self.zo_forward(model, inputs)

                        # Exchange loss within the GPU pair
                        loss_pos, loss_neg = zo_opt.exchange_loss(loss_local)
                        projected_grad = (loss_pos - loss_neg) / (2 * self.args.zo_eps)
                        local_grads.append(projected_grad)

                        # Restore parameters (undo perturbation)
                        zo_opt.restore_parameters_by_named(self.named_parameters_to_optim, random_seed)

                    # Gather all K gradients across all GPU pairs
                    grads = zo_opt.gather_grads_across_pairs(local_grads, my_directions, self.args.n)

                else:
                    # === Original sequential gradient computation (single GPU) ===
                ...
                if self.args.dp_epsilon > 0:
                    ...
                    # Deterministic noise across all ranks for parallel ZO
                    if is_parallel_zo:
                        torch.manual_seed(self.args.seed * 200000 + self.state.global_step + 1)
                    ...
                    for j in range(self.args.n):
                        ...
                        for name, param in self.named_parameters_to_optim:
                            ...
                            if "bias" not in name and "layer_norm" not in name and "layernorm" not in name:
                                param.data = param.data - self._get_learning_rate() * (noisy_mean_grad[j] * z + args.weight_decay * param.data)
                            else:
                                param.data = param.data - self._get_learning_rate() * (noisy_mean_grad[j] * z)
                            del z
